  ...
  return {
    ...
    Order() {
      ...
      const csrftoken = document
        .querySelector("div#csrf input[name='csrfmiddlewaretoken']")
        .getAttribute("value");
    ...
    addToFav(productID) {
      ...
      axios.defaults.headers.common["X-CSRFTOKEN"] = csrftoken;
      axios
        .post("/api/add-to-fav", { productID })
        .then(({ data }) => {
          if (data.isOk) {
            this.prodAddedToFavOverlay = true;
          } else if (data.error_type == "product-in-fav") {
            notyf.error("Ce produit est déjà dans vos favoris !");
    ...
    fetchCart() {
      ...
      axios
        ...
        .catch((err) => {
          console.log(err);
          notyf.error(
            "Une erreur s'est produite durant le chargement du panier."
          );
        })
        .finally(() => {
...
window.addEventListener("load", function () {
  ...
  let most_sold_products_length =
    most_sold_products_el.getAttribute("pr-count");
  ...
  let most_popular_products_length =
    most_popular_products_el.getAttribute("pr-count");
  let most_popular_loader = document.querySelector("#most-popular-loader");

  let glide_new_products = new Glide(
    document.querySelector(".glide-new-products"),
    getGlideOptions(new_products_length)
  ).mount();
  new_product_loader.classList.add("hidden");
  new_products_el.classList.remove("h-0");
  new_products_el.classList.remove("invisible");

  let glide_most_sold = new Glide(
    document.querySelector(".glide-most-sold"),
    getGlideOptions(most_sold_products_length)
  ).mount();
  ...
  let glide_most_popular = new Glide(
    document.querySelector(".glide-most-popular"),
    getGlideOptions(most_popular_products_length)
  ).mount();
